...
SearchResult depth_limited_negamax(AlphaBetaContext* ctx, State* state,
    uint8_t depth, double alpha, double beta)
{
    if (out_of_time(ctx)) {
        ctx->timed_out = true;
    ...
    for (uint16_t i = 0; i < child_count; i++) {
        ...
        SearchResult result = depth_limited_negamax(ctx, child, depth - 1, -beta, -alpha);
